...
public enum TranscriptFact: Equatable, Sendable {
    ...
    /// The call failed or was refused, so no background work survives its return.
    case callFailed(activityID: String, at: Date)
    ...
    public func ends(activityID: String?, agentID: String?) -> Bool {
        switch self {
        case let .callReturned(id, _), let .callFailed(id, _), let .workEnded(id, _):
            return SessionReducer.activityEndsWait(
                awaitedActivityID: activityID,
...
public enum TranscriptReader {
    ...
    private static func claudeFacts(in record: [String: Any], at: Date) -> [TranscriptFact] {
        ...
        let isUser = record["type"] as? String == "user" && message["role"] as? String == "user"
        let isAssistant = record["type"] as? String == "assistant" && message["role"] as? String == "assistant"
        // A task notification is a system-origin user message, not text generated by the
        // assistant or typed by the person. Actual notifications carry string content.
        if isUser, record["promptSource"] as? String == "system",
            (record["origin"] as? [String: Any])?["kind"] as? String == "task-notification",
            let text = message["content"] as? String,
            let raw = taskNotificationToolUseID(in: text)
        {
            return [.workEnded(activityID: HookCaptureRedactor.label(forRawIdentifier: raw), at: at)]
        }
        guard let parts = message["content"] as? [[String: Any]] else {
            return []
        }
        // Older records without this reference are indistinguishable from a literal user
        // quotation. Losing an ambiguous cancellation is safer than inventing one.
        if isUser, let interrupted = record["interruptedMessageId"] as? String, !interrupted.isEmpty,
            parts.count == 1, parts[0]["type"] as? String == "text",
            let text = parts[0]["text"] as? String,
            text == "[Request interrupted by user]" || text == "[Request interrupted by user for tool use]"
        {
            return [.turnInterrupted(at: at)]
        }
        ...
        for part in parts {
            ...
            case "server_tool_use" where isAssistant && part["name"] as? String == "advisor":
            ...
            case "tool_result" where isUser, "advisor_tool_result" where isAssistant:
                if let raw = part["tool_use_id"] as? String {
                    let id = HookCaptureRedactor.label(forRawIdentifier: raw)
                    facts.append(
                        part["is_error"] as? Bool == true
                            ? .callFailed(activityID: id, at: at) : .callReturned(activityID: id, at: at))
                }
            default:
                continue
            }
    ...
    private static func taskNotificationToolUseID(in text: String) -> String? {
        guard text.hasPrefix("<task-notification>"), text.contains("</task-notification>"),
            let status = element("status", in: text), ["completed", "failed", "killed"].contains(status)
        else {
            return nil
        }
