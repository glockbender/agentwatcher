...
/// The overflow counter exists precisely for the case where the scrollers have auto-hidden.
final class HiddenRowCountTests: XCTestCase {
    ...
    func testNothingIsHiddenWhenEveryRowFits() {
        XCTAssertEqual(hiddenRowCount(rowFrames: rows(3), visibleRect: visible), 0)
    }

    func testRowsBelowTheFoldAreCounted() {
        XCTAssertEqual(hiddenRowCount(rowFrames: rows(6), visibleRect: visible), 3)
    }

    func testAPartlyVisibleRowCountsAsHidden() {
        let straddling = NSRect(x: 0, y: 90, width: 300, height: 30)

        XCTAssertEqual(hiddenRowCount(rowFrames: [straddling], visibleRect: visible), 1)
    }
    ...
    func testARowWiderThanTheViewIsNotHidden() {
        ...
        XCTAssertEqual(hiddenRowCount(rowFrames: [wide], visibleRect: visible), 0)
    }

    func testAHorizontallyScrolledViewStillHidesNothingVertically() {
        let scrolledRight = NSRect(x: 200, y: 0, width: 300, height: 100)

        XCTAssertEqual(hiddenRowCount(rowFrames: rows(3), visibleRect: scrolledRight), 0)
    }
    ...
    func testARowEndingExactlyAtTheFoldIsVisible() {
        ...
        XCTAssertEqual(hiddenRowCount(rowFrames: [flush], visibleRect: visible), 0)
    }

    func testAnEmptyListHidesNothing() {
        XCTAssertEqual(hiddenRowCount(rowFrames: [], visibleRect: visible), 0)
    }
