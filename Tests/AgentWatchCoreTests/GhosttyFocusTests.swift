...
struct GhosttyFocusTests {
    ...
    /// it.
    @Test func theTabWhoseTitleEndsWithTheSessionNameIsTheOne() {
        let decision = GhosttyFocus.decision(among: tabs, sessionName: "Следующие задачи по плану")

        #expect(decision == .ask(terminalID: "EA251683-CAB4-429D-A26C-3F074120F240"))
    }
    ...
    @Test func aDifferentGlyphInFrontChangesNothing() {
    ...
    @Test func twoMatchesAreRefusedRatherThanGuessedBetween() {
        let twins = [
        ...
        #expect(GhosttyFocus.decision(among: twins, sessionName: "Одно и то же имя") == .decline(.severalTabsMatch))
    }

    @Test func nothingMatchingIsSaidAsItself() {
        #expect(GhosttyFocus.decision(among: tabs, sessionName: "Что-то другое") == .decline(.noTabMatches))
    }
    ...
    @Test func aSessionWithNoNameYetHasNothingToMatchOn() {
        #expect(GhosttyFocus.decision(among: tabs, sessionName: nil) == .decline(.sessionHasNoName))
        #expect(GhosttyFocus.decision(among: tabs, sessionName: "   ") == .decline(.sessionHasNoName))
    }

    /// An empty name would otherwise be a suffix of every tab, and the first one would win.
    @Test func anEmptyNameDoesNotMatchEverything() {
        #expect(GhosttyFocus.decision(among: tabs, sessionName: "") == .decline(.sessionHasNoName))
    }
    ...
    @Test func onlyAUUIDCanBeWrittenIntoAScript() {
        #expect(GhosttyFocus.isAddressableTerminalID("EA251683-CAB4-429D-A26C-3F074120F240"))
        #expect(!GhosttyFocus.isAddressableTerminalID("\" to quit -- "))
        #expect(!GhosttyFocus.isAddressableTerminalID(""))
    }

    /// Only what a person can act on reaches the log; the ordinary case stays quiet.
    @Test func onlyActionableRefusalsAreWorthALine() {
        #expect(JetBrainsFocusRefusal.notAJetBrainsIDE.attempt == .unaddressable)
        #expect(JetBrainsFocusRefusal.daemonMissing.attempt != .unaddressable)
        #expect(GhosttyFocusRefusal.noTabMatches.attempt != .unaddressable)
    }
}
