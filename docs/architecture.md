...
`thread`, `counters`, `context` и `gap`. Таймер, лампа, иконка агента и разрыв есть в каждой
строке; остальные появляются, только когда сессии есть что в них положить, — и окно настроек
говорит это словами напротив каждой части, коротко в колонке и целиком в подсказке.
...
Настройка `showsSessionTopic` этим шаблоном поглощена. Её значение переносится ровно один раз, и
переносит его засев умолчаний: он пишет только те ключи, которых в файле нет, поэтому снятая
галочка решает судьбу шаблона на первом запуске после обновления и больше никогда. Флаг читала не
только строка — карточка при наведении брала его, чтобы не назвать сессию, которую строки только
что перестали называть; теперь обе спрашивают шаблон, и ответ один, а не два.
