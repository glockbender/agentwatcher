...
GitHub видит то же, что любой сайт: адрес и IP. Спрашивается список релизов, а не `/releases/latest`:
последний пропускает пометку pre-release, а пока проект в альфе релизы именно такие — замерено, адрес
ответил 404 на единственный существующий релиз.
...
Третье — человек убрал её сам; тогда она не возвращается, пока жив этот процесс, — но только до
конца этого запуска: список убранных строк лежит в памяти и на диск не пишется, поэтому после
перезапуска в виджете снова видно всё, что работает. Это решение, а не упущение. Список пробовали
