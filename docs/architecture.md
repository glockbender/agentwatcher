...
Шесть групп: формат строки (см. предыдущий раздел), лампа (строка на фазу — название, цвет,
движение, и подсказка из колонки «Значение» таблицы §7), фон (палитра из десяти), непрозрачность,
размер и сочетание клавиш. Две кнопки возвращают вид приложения — строке и лампе.

Содержимое прокручивается и окно тянется за размер, как в окне `Tooling`, и по той же причине:
тринадцать строк формата подняли высоту выше, чем даёт экран ноутбука — замерено, 1290 точек
против 1079, — а окно не поднимают над строкой меню, поэтому нижние группы оказывались за нижним
краем без единого способа до них добраться.
