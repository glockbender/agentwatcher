...
> **Состояние реализации на 2026-09-07.** Работает локальная alpha: AppKit HUD, меню, Unix-socket ingress, fail-open sender, нормализация Claude/Codex lifecycle events, чтение транскрипта, debug-window и базовый best-effort focus/закрытие CLI-сессии. Встраивание в тулинг тоже работает и управляется из окна `Tooling…`: плагин хуков для Claude, слияние для Codex, обёртка статус-строки. Постоянная история, полноценный watchdog, обновление, статистика и локальные модели ещё не реализованы. Детальный порядок — в [плане реализации](implementation-plan.md).
...
- признак «переживает ход» у активности из `SubagentStart` работает — она попадает в строку
  родителя;
- а вот **вызовы инструментов внутри сабагента в строку родителя не попадают**: их отбрасывает
  отправитель хука. Сначала было наоборот — работа сабагента считалась работой родителя, и вызов
  оболочки внутри сабагента открывал активность в его строке. Замер показал, чего в этом
  рассуждении не хватало: закрыть такую активность нечем. За пять минут, в которые сама сессия не
  выполнила ни одной команды, на её строке открылось 48 вызовов оболочки — и ни один из 48 не
  закрылся никогда. `PostToolUse` для них не приходит, а результат вызова сабагент пишет в свой
  файл `…/subagents/agent-….jsonl`, который приложение читать не имеет права (§14). Строка
  набирала по сотне «работающих» команд за длинный ход, и вычищал их только конец хода.

Отбрасывает именно отправитель, и это единственное место, где вопрос вообще можно задать: судить
приходится по пути транскрипта из payload, а путь по проводу не ходит (§15) — на той стороне
отличить свой вызов от чужого нечем. Отбрасываются только `PreToolUse`, `PostToolUse`,
`PostToolUseFailure` и `PermissionDenied` — см. `SubagentToolCall` в отправителе.
`SubagentStart`/`SubagentStop` проходят: ими строка и говорит, что сабагент работает, и это
остаётся единственным, что она о нём говорит. Проходит и `PermissionRequest`: сессия, у которой
спрашивают разрешение, ждёт человека, кто бы ни сделал вызов.

Поле `SessionActivity.parentID` объявлено и не заполняется. Пока вложенных вызовов не приходит,
вкладывать нечего; `agent_id` остаётся готовым ключом, если однажды появится источник, из которого
конец такого вызова будет виден.
...
Выход `WaitingUser --> Executing` уточнён. Ожидание привязано к конкретному вызову инструмента
(`awaitedActivityID`): его снимает завершение именно этого вызова, начало любого нового вызова или
конец хода. Завершение параллельного вызова — не снимает. Причина простая: ход остановлен, пока
человека спрашивают, поэтому ничего нового не запускается до ответа, а вот уже запущенные вызовы
продолжают завершаться — и раньше любое такое завершение возвращало сессию в «работает», пока диалог
висел на экране.
...
цвет строки. Источник и клиент в строку не пишутся — это отдельные иконки перед ней. Давность
последнего события тоже не пишется словами: её показывает таймер строки.
...
Карточка привязана к идентификатору сессии, а не к вьюхе строки: событие пересобирает список, и
карточка, привязанная к вьюхе, исчезала бы прямо во время чтения — именно у той занятой сессии,
...
У продукта две поверхности с разными задачами.
...
Порядок слева направо один и тот же в каждой строке, поэтому колонки читаются сверху вниз. Кнопка
перехода идёт первой и занимает постоянную ширину независимо от того, есть ли у сессии кнопка
удаления: иначе всё правее неё съезжало бы от строки к строке.
...
Источник и клиент всегда значки: настоящие иконки установленных приложений для Claude и Codex,
системные символы для desktop и CLI. Слова на них тратили около пятнадцати знаков в каждой строке.
...
рамку окна сама, а середина виджета нажатия не берёт — за неё виджет по-прежнему перетаскивается. На
время перетаскивания полоса остаётся подсвеченной, даже если указатель ушёл за пределы виджета:
...
Если сессий больше, чем помещается, список прокручивается и появляется счётчик скрытых строк:
...
Рисуется он поверх списка, а не под ним, и это исправление: в потоке счётчик забирал у списка
полосу высоты, то есть сам был отчасти причиной того, о чём сообщает — пока он висел, виджет
показывал на строку меньше. Теперь список всегда занимает всю высоту виджета, а счётчик стоит
капсулой у нижней кромки списка, прижатой к правому краю: овал потому, что он лежит поверх чужой
строки — текст поверх текста читался бы как часть этой строки, а прямоугольник как её поломка.
Поле вокруг надписи минимальное, шрифт меньше всего остального в виджете, и нажатий капсула не
берёт: под ней живая строка, которую можно навести, прокрутить и убрать.
...
включая то, что зависит не от события, а от времени: появилась ли у строки кнопка `×` после
получаса молчания. Функция `rowListUpdate(from:to:)` сравнивает старый список моделей с новым и
...
Три группы: лампа (строка на фазу — название, цвет, движение, и подсказка из колонки «Значение»
таблицы §7), фон (палитра из десяти) и непрозрачность. Кнопка возвращает лампе вид приложения.
...
В начале каждой строки стоит небольшая кнопка `↗`. Она видна всегда и имеет постоянную ширину: с неё
начинаются колонки строки, и кнопка, появляющаяся по наведению, сдвигала бы всё правее себя. Кнопка
возвращает пользователя к приложению, окну и, когда это возможно, точной вкладке сессии.

Для этого к сессии считается `SessionLocator` — не сохранённый, а спрошенный в момент нажатия:

```text
SessionLocator
├── applicationName   имя приложения-хозяина
├── projectName       имя окна проекта, когда оно известно: имя каталога, не путь (§15)
├── tabName           имя вкладки; оно же имя сессии — агент пишет его сам
└── precision         выводится из полей, а не объявляется
```

Точность — четыре ступени, и каждая означает ровно то, что приложение может назвать:
`nothing` (хозяина нет), `application`, `projectWindow`, `tab`.

**Считается в момент нажатия, а не запоминается.** Прошлое устройство разрешало хозяина один раз —
на первом хуке или пока приложение ещё стартовало — и держало этот ответ всю жизнь строки. Любая
осечка в тот единственный момент гасила кнопку навсегда, и вероятнее всего это доставалось самой
...
**Кнопка `↗` нажимается всегда.** Серый control говорит «до этой сессии не добраться», что почти
никогда не правда: хозяин, как правило, на месте. Насколько близко доводит одно нажатие — говорит
карточка строки: «↗ brings GoLand forward — the mcp-hub window, terminal tab named above», а когда
хозяина действительно нет — «No window to bring forward».

Особый случай — **фоновая сессия** (Claude Code запускает такие сам, в своём pty под
`claude bg-pty-host`). Окна у неё нет и не появится: замерено, дерево процессов такой сессии
упирается в `launchd`, приложения над ней нет вовсе. Зато у неё есть дверь: `claude attach <job id>`
показывает сессию в любом терминале, а `Ctrl+Z` там возвращает в shell, не останавливая её. Поэтому
нажатие для такой строки не поднимает окно, а открывает новую вкладку Ghostty и печатает в неё эту
команду; кнопка была серой, пока двери не было. Второе нажатие вкладку не дублирует: пока процесс
`claude attach <job id>` работает, Ghostty называет его вкладку самой командой, и если такая вкладка
ровно одна или больше — поднимается первая из них. Заголовок без процесса за ним не в счёт: замерено,
...
`~/.claude/sessions/<pid>.json`, в момент нажатия: запись появляется, когда сессию уводят в фон, а
...
может вернуть объект вкладки, отвечая −1708 на вкладку, которая уже есть. Карточка говорит: «↗ opens
it in a new Ghostty tab with `claude attach` — a background session has no window of its own», в
первой строке карточки стоит `Background` вместо `CLI`. Отправитель узнаёт такую сессию по команде
своего процесса-предка (`AgentProcessLocator`), значок в строке — перечёркнутое окно.

Проект берётся из файлов самой IDE: `product-info.json` в бандле даёт имя каталога настроек, а
`recentProjects.xml` — список проектов; рабочий каталог агента ищется среди них — сначала среди
открытых, потом самым длинным совпадением пути. Ни одного разрешения macOS это не стоит, и полный
путь наружу не выходит — §15. Хост без такого списка (Ghostty, Terminal.app) отвечает пустым, и это
правильный ответ.

Имя проекта попадает в locator **только если файл называет проект открытым**, потому что оно
используется, чтобы указать на окно. Файл пишется не сразу: однажды он называл открытым проект,
закрытый три часа назад. Поэтому «окно проекта X» — утверждение слабее остальных, и произносится
оно только когда есть хоть какое-то основание. Имя проекта самой сессии на карточке и так есть,
строкой выше, и оно приходит от неё самой.

Если locator устарел, приложение не должно угадывать и фокусировать похожую сессию.
...
Сторона Agent Watch написана и работает так: `↗` сначала поднимает приложение — как и раньше, — и
...
по этому пути не приходит ничего: что сделал плагин, видно только в журнале IDE. Поэтому кнопка
...
меняет ответ, а запомненный ответ — это ровно та ошибка, что была в кнопке фокуса.
...
отношения не имеет — там ключом остаётся номер процесса; это только признак того, что кнопка
сработает, а кнопка, которая не сработает, хуже отсутствующей.
...
открытой активности у неё нет, и ни один из двух источников не сказал ничего две минуты. «Сказал»
здесь шире, чем «сообщил факт»: любой прирост транскрипта считается за услышанное, даже если ни
одной строки из него читатель не взял. Ход, который две минуты думает или печатает длинный ответ, не
...
сборке без бандла: у неё нет версии, которую можно с чем-то сравнить.
...
GitHub видит то же, что любой сайт: адрес и IP. Спрашивается последний завершённый релиз
...
распаковывает `ditto` и подменяет свой бандл атомарной заменой, после чего запускает новую копию и
выходит. Порядок именно такой, потому что второй экземпляр не поднимется, пока первый держит сокет:
новую копию открывает короткий `sh`, переживающий этот процесс.
...
- журнал отладки дописывает одну строку, а не переписывает файл целиком на каждое событие; полная
  перезапись случается раз на двести записей, когда файл вырастает вдвое против своего предела;
...
Открытые вызовы и предупреждения не возвращаются, и почти всякая фаза тоже — сессия приходит в
`disconnected`, то есть «ничего не слышно», и первый же настоящий хук говорит, что она делает.
...
работа), при пробуждении Mac и при открытии меню. Замер: перечисление 156 процессов — 0,03 мс,
...
(`SessionStateEngine.retireSessionsSuperseded`). Убирается только закрытая, и это же защищает от
...
`sessions-remembered.json` со сессиями последнего запуска.
...
только закрывает, но и **открывает**. Открывает он её узким событием, не трогающим фазу:
...
`floor` — это число из меню (3/5/10), `idle` вдвое больше. Правило вынесено в
`TranscriptReadSchedule` и проверяется без диска и без таймера.
...
Чтение начинается с конца файла, а не с начала. Файл хранит всю историю сессии, и разбор с
начала закрыл бы вызовы, завершившиеся час назад, и увёл бы сессию в покой по прерыванию,
случившемуся до запуска приложения.

Исключение одно: **первая запись файла**, прочитанная один раз в момент, когда файл найден.
...
пока хотя бы одна сессия заявляет о работе: фаза, объясняющая собственную тишину, не может
дать ничего нового, и опрос ради неё был бы опросом в состоянии покоя. Интервал задаётся в
