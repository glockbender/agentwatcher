...
Отбрасывал это отправитель — `SubagentToolCall`, — и тогда это было единственное место, где вопрос
вообще можно было задать: судить приходилось по пути транскрипта из payload, а путь по проводу не
ходит (§15). Отбрасывались `PreToolUse`, `PostToolUse`, `PostToolUseFailure` и `PermissionDenied`;
`SubagentStart`/`SubagentStop` проходили, ими строка и говорит, что сабагент работает. Проходил и
`PermissionRequest`: сессия, у которой спрашивают разрешение, ждёт человека, кто бы ни сделал вызов.

**На 2.1.272 это условие не выполняется ни разу, и то, ради чего оно писалось, исчезло само.**
Замерено: у хука из сабагента `transcript_path` — путь *родительской* сессии, точно такой же, как у
хуков основного потока; собственный файл сабагента приходит отдельным полем `agent_transcript_path`
и только у `SubagentStop`. Значит проверка по пути больше никого не узнаёт, и вызовы сабагента
доходят до приложения. Но они и **закрываются**: `PostToolUse` от сабагента теперь приходит, с тем
же `agent_id`, что и его `PreToolUse`. Незакрываемых вызовов, из-за которых строка набирала по сотне
«работающих» команд, больше нет.

Что осталось от этого — мёртвая проверка в отправителе и вопрос, стоит ли её убирать. Она fail-open
и ничего не ломает, но её док-комментарий описывает версию Claude Code, которой уже нет. Вопрос, в
чью работу считать вызов сабагента — родителя или его собственную, — теперь можно задать и на
стороне приложения: `agent_id` доезжает (`agent-integration.md` §1в), а раньше не доезжал ничего.
...
(`awaitedActivityID`) **и к тому, кого спросили** (`awaitedAgentID`): его снимает завершение или
ошибка именно этого вызова, начало нового вызова тем же агентом, конец самого этого агента
(`SubagentStop`), а для диалога основного потока — ещё и конец хода. Поздно прочитанное начало
