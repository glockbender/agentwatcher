...
Поиск видит не каждого агента, и это две известные дыры, а не правило. Процесс, чей исполняемый
файл уже удалило обновление Claude Code, `proc_pidpath` не называет (`ENOENT`), и строки у него нет:
так остался невидимым второй висящий агент, запущенный 14 сентября на 2.1.270. И процесс, чей файл
ядро называет по второму имени — `~/.local/share/claude/ClaudeCode.app/Contents/MacOS/claude`, его
заводит фоновая сессия, — правилом пути (`…/claude/versions/<версия>`) не узнаётся, даже если его
запустили обычным образом; у такой сессии строка есть, только пока от неё приходят хуки. Почему ядро
так делает — `agent-integration.md` §1б, «Путь к программе называет файл, а не запуск». Обе дыры
записаны в плане.
