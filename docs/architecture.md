...
И только вниз: `▾ N more` стоит под последней строкой и обещает то, до чего докрутят дальше.
Раньше в счёт шли и строки, ушедшие вверх, поэтому внизу длинного списка капсула продолжала
объявлять пять уже прочитанных сессий — под стрелкой, показывающей в пустоту. Теперь в самом низу
её нет вовсе.
...
капсулой в правом нижнем углу: овал потому, что он лежит поверх чужой строки — текст поверх текста
читался бы как часть этой строки, а прямоугольник как её поломка.

Капсула уходит в собственную рамку виджета, а не встаёт вровень со строками: под ней правый нижний
угол строки, где живёт её `×` — единственный способ убрать остановившуюся сессию. Строки свой отступ
сохраняют, в рамку сдвигается только капсула, и каждая точка этого сдвига — точка кнопки, в которую
можно попасть. Замерено: раньше капсула закрывала сам знак `×` целиком, открытой оставалась только
прозрачная кромка рамки кнопки; теперь открыта её большая часть — 14 точек из 22 без блока usage и
12 с ним. Ниже она опускается ровно
настолько, насколько позволяет то, что под списком: край виджета — или разделитель блока usage,
который счётчик не перекрывает. Поле вокруг надписи минимальное, шрифт меньше всего остального в
виджете, и нажатий капсула не берёт: под ней живая строка, которую можно навести, прокрутить и
убрать.
...
Для этого к сессии считается `SessionLocator` — не сохранённый, а спрошенный в момент клика:
...
почти никогда не правда: хозяин, как правило, на месте. Насколько близко доводит один клик — говорит
карточка строки: «Click brings GoLand forward — the mcp-hub window, terminal tab named above», а
когда хозяина действительно нет — «No window to bring forward».
...
процесс жив, он и есть окно сессии: строка носит значок терминала, карточка обещает «Click brings
Ghostty forward», а клик поднимает Ghostty и вкладку, названную именем сессии, как для любой
терминальной сессии. Клик через дверь открыл бы сессию второй раз рядом с вкладкой, где она уже
...
в первой строке карточки стоит `Background` вместо `CLI`, значок в строке — перечёркнутое окно. Как
...
хуку с именем исходной складывается в её строку (`SessionStateEngine.foldContinuedRow`): остаётся
