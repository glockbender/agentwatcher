...
stateDiagram-v2
    ...
    Idle --> TerminalClosed: the terminal was closed, the agent stayed
    Completed --> TerminalClosed: the terminal was closed, the agent stayed
    WaitingUser --> TerminalClosed: the terminal was closed, the agent stayed
    Disconnected --> TerminalClosed: the terminal was closed, the agent stayed
    TerminalClosed --> SessionClosed: the agent exits — after a click, or on its own
```

`TerminalClosed` агент не сообщает: эту фазу находит приложение, спрашивая ядро о процессе.
Терминал, в котором шла сессия, закрыли, а агент не завершился. Замерено на Claude Code 2.1.280 во
вкладке PyCharm и 2.1.270 во вкладке GoLand: агент висит в `tcsetattr`, ожидая, что терминал
примет его последний вывод, а PyCharm держит открытыми обе стороны терминала. Воспроизводится это
состояние так: держать терминал открытым, ничего из него не читать и послать оболочке `SIGHUP`, — и
тогда ни `SIGTERM`, ни `SIGKILL` агента не завершают (`agent-integration.md`, «Терминал закрыли, а
агент остался»).
...
| Терминал закрыт | `#FF453A` красный | ровно горит | Терминал, в котором шла сессия, закрыли, а агент не завершился: он висит, ожидая, что терминал заберёт его последний вывод. Клик по строке его завершает |
...
собственном фоне виджета: одном из десяти фиксированных, половина из которых светлые, или на цвете,
который человек выбрал сам. То есть
...
на собственном фоне виджета — одном из десяти, половина которых светлые, или на своём цвете
человека. Взяты `systemYellow` и
...
высота рамки по пяти размерам от 100 до 200% — 19, 23, 23, 23, 23, то есть после 125% она расти перестаёт и кнопка
...
Порог — четверть от силы рабочего знака, и проверяется он на всех размерах, которые предлагает
ползунок. Замер на семи шагах по четверти, на тёмном и светлом виджете: приглушённый держит от 0,49
до 0,74, а версия с текстом держала 0,05.
...
обводится оранжевым и пульсирует пять секунд поверх содержимого, нажатий не перехватывая.

Тот же контур зажигают ещё три действия, и все по одной причине: виджет меняется, пока человек
смотрит не на него. Ползунок размера в окне настроек — смотрят на ползунок, а меняется маленькое
окно где-то ещё. `Reset Widget Size` и `Reset Widget Position` — смотрят в меню, и второе вообще
нажимает тот, кто виджет потерял: середина главного экрана — это всё ещё место, которое надо
найти. Спрятанный виджет при этом показывается, по той же причине.

У ползунка есть условие — размер должен действительно измениться, иначе контур загорался бы на
запуске, когда контроллер рождается с уже сохранённым размером. У двух строк меню условия нет:
нажатие на строку меню человек сделал сам, и «вот он я» — половина ответа, за которой он туда и
пришёл. Отдельной метки для всего этого нет намеренно: «вот он я» в приложении говорится одним
способом.
...
целую. Замерено на шагах по четверти: со 100% и выше под любой из капсул остаётся не меньше половины
...
- **Other** — фон (палитра из десяти и свой цвет на цветовом круге), непрозрачность и сочетание
  клавиш.
...
**Свой цвет фона.** Под двумя рядами палитры — строка `Custom` с системным колодцем цвета, тем же,
что у лампы. Нажатие на него — выбор, как нажатие на образец: фоном сразу становится цвет колодца,
и открывается системная панель цветов на странице цветового круга, на какой бы странице её ни
оставили. Панель одна на всё приложение, колодцы лампы делят её с этим, поэтому круг включается при
каждом открытии, а не один раз. До первого выбора колодец показывает фон, который сейчас в
работе, — круг открывается на том, как виджет выглядит, и первое нажатие виджет не меняет. Выбран
свой цвет — кольцо акцента обводит колодец, а не образец. Прозрачности в панели нет: её задаёт
отдельный ползунок, а файл хранит цвет без альфы, и выбранная в панели альфа пропала бы молча.

Хранится свой цвет отдельным ключом `widgetBackgroundCustomColor` (`#RRGGBB`) рядом с
`widgetBackground = custom`. Отдельным — чтобы он пережил переход на пресет: колодец продолжает его
показывать, и вернуться к нему можно одним нажатием, а не поиском на круге. До первого выбора ключ
записан пустой строкой, как пустое сочетание клавиш. `custom` без читаемого цвета за ним читается
как Graphite — по тому же правилу, что опечатка в имени пресета. Круг сообщает о каждом оттенке под
указателем, а каждая запись перестраивает все строки виджета, поэтому цвет, не изменившийся с
точностью файла, не записывается повторно.

Текст на своём цвете — тёмный или белый — выбирается по контрасту в смысле WCAG (отношение яркостей
двух цветов, от 1 : 1 до 21 : 1): берётся тот из двух цветов текста виджета, который отличается от
фона сильнее. По расчёту граница — относительная яркость 0,224: `#828282` ещё с белым текстом,
`#838383` уже с тёмным, и на самой границе оба дают около 3,8 : 1 — это худший контраст для любого
цвета с круга. Раньше десять пресетов делились на тёмные и светлые списком; теперь то же правило
считает и для них, а тест держит его согласным с рядами палитры. Отрисовкой проверено на четырёх
цветах, включая чистый красный — он лежит почти на границе (0,213) и получает белый текст.

Тем же решением выбирается и тема, в которой AppKit рисует элементы управления на этом фоне:
светлому фону — светлая (`aqua`), тёмному — тёмная (`darkAqua`), `WidgetBackground.controlAppearance`,
а список строк задаёт её себе сам. Иначе рамку кнопки `×` система рисует в теме Mac, а не виджета:
при светлой теме системы тёмный виджет показывал почти чёрный `×` на тёмной рамке. Заметили это не
глазом, а по CI: `testEitherCounterLeavesTheDismissMarkShowing` падал на машине GitHub с того дня,
как был написан, а здесь падал только с принудительно светлой темой, и отрисовка в ней показала
тот самый `×`. Что машина GitHub рисует в светлой теме — вывод из этого (у macOS она по умолчанию),
а не замер.

Два факта про `NSColorWell`, найденные замером. `NSColorWell(style:)` — фабрика: вызванная на
подклассе, она возвращает обычный `NSColorWell`, поэтому переопределение `activate` не срабатывало —
клик открывал панель на прежней странице и ничего не выбирал. Колодец создаётся через `init()`,
который даёт тот же стиль по умолчанию. И поймать эту ловушку можно только настоящим кликом —
событием мыши, отправленным окну: вызов `activate(true)` из Swift доходит до переопределения
напрямую и проходил тест с ловушкой на месте.
...
Ползунок «Size» задаёт масштаб виджета: от 50 до 200 процентов шагом в пять — 31 засечка, и ручка
встаёт только на них. Шаг сначала был в четверть и оказался слишком крупным: четверть виджета за одно
движение, а нужный размер часто лежит между двумя соседними засечками. Пять процентов — то, о чём
попросил владелец, и это по-прежнему засечки, а не свободный ползунок: свободный предложил бы сотни
положений, из которых соседние неразличимы. Засечки заодно и есть механизм — AppKit останавливает
ручку на засечке и отдаёт её значение, поэтому в настройках всегда лежит один из размеров списка.
Округление к ближайшей засечке лежит ещё и в самом хранилище настроек, потому что ползунок не
единственный, кто туда пишет: отредактированный рукой файл настроек — тоже, и записанные в нём 1,07
дали бы виджет размера, про который окно настроек сказало бы «105%».

Сам ползунок `Size` — единственный элемент окна, растянутый во всю ширину вкладки: он тянется за
окном, когда окно тянут. У остальных ширина что-то значит сама по себе — колонка сетки, образец
цвета, — и растягивать их значило бы только развести их части друг от друга. У этого значение имеет
ровно одно: как далеко разъехались засечки. На 260 точках, которые у него стояли раньше, 31 засечка
сливалась в частокол, а ширина в окне уже была — её держит сетка частей строки. Ползунок `Opacity`
остался прежней ширины: засечек у него нет, и разводить ему нечего.

Растяжение потребовало починить соседнюю вещь. Содержимое вкладки — вертикальный стек, выровненный по
левому краю, и правый край он не держал ни за что: ширина вкладки получалась как левый отступ плюс
самая широкая группа, а правый отступ окно добавляло к своей ширине руками. Секция, ширина которой
равна ширине вкладки минус оба отступа, в такой схеме отодвигала окно ещё на один отступ вправо.
Теперь каждая секция сама держится ограничением «не дальше правого края минус отступ», и окно
складывает ширину без поправок.

Ползунок, а не меню, по той же причине, по которой ни один элемент этого окна не ждёт кнопки
«применить»: виджет виден, пока окно открыто, и человек тянет ручку, пока строки не станут такими,
как ему нужно, вместо того чтобы выбрать число и потом проверять. Запись происходит, только когда
значение действительно изменилось: ползунок сообщает о себе на каждом кадре перетаскивания, а каждая
запись перестраивает все строки виджета.
...
`anApplication`, если сессию держит приложение и клик его поднимет, `nowhere`, если не держит
никто, и `closedTerminal` — терминал сессии закрыт, и клик завершит оставшегося агента (ниже).
Отдельный тип, а не `Bool`: «поднимать нечего» — это факт о сессии, а `false` на месте вызова
говорит только, что что-то неверно.
...
нет, про новую вкладку, если окна у сессии нет вовсе, и «Click ends it…», если терминал сессии
закрыт, а агент остался (ниже). Раньше она описывала и обычный случай —
...
отправитель узнаёт такую сессию (`ClaudeProcessRules.clientKind`), сказано в начале этого случая.

Второй особый случай — **терминал закрыт** (фаза `TerminalClosed`, §7): вкладку закрыли, а агент
остался висеть. Дерево процессов по-прежнему ведёт к IDE, и обычный клик поднимал её окно, в
котором вкладки этой сессии уже нет. Теперь клик агента завершает: приложение открывает устройство
терминала, которое агент держит на своих дескрипторах (`/dev/ttysNNN`), и сбрасывает непрочитанный
вывод (`tcflush`), — ожидание в `tcsetattr` кончается, агент выходит сам, и строку закрывает
обычный наблюдатель за выходом процесса. Почему клик, а не подсказка с `kill` —
[ADR-0013](adr/0013-a-click-ends-an-agent-its-closed-terminal-left-behind.md). Три правила вокруг
этого клика:

- **клик, который завершает процесс, объявлен заранее.** Карточка такой строки говорит «Click ends
  it…» и печатает команду, которой то же самое делается руками. Печатает, а не предлагает
  скопировать: карточка не принимает мышь (выше), и выделить в ней нечего;
- **первый клик по строке, ещё не помеченной, только помечает её.** Сканирование могло не успеть:
  вкладку закрыли минуту назад. Такая строка выглядела обычной, и её карточка ничего не обещала —
  значит, и завершать по этому клику нельзя;
- **перед сбросом всё спрашивается заново**: жив ли процесс, потерян ли его терминал, какое
  устройство он держит. Пометка могла устареть, а терминал вышедшего агента — достаться новой
  вкладке, чей вывод выбрасывать нельзя. Открывается только путь вида `/dev/ttys<цифры>`: он
  прочитан из чужого процесса, а открывается на запись.
...
Различает их первое слово команды — `ClaudeProcessRules.isHelperCommand`. В списке то, что
...
**Живой процесс — ещё не сессия, до которой можно дойти.** Так нашлась строка, с которой начался
этот случай (2026-09-22): вкладку PyCharm закрыли, `claude` в ней не вышел, и после перезапуска
приложения поиск поставил ему строку — без имени, потому что в сессии не было ни одного запроса, с
фазой `no signal` и кликом, поднимавшим PyCharm. Теперь тот же поиск спрашивает у каждой строки
`cli`, есть ли ещё у её агента терминал, и строке без терминала ставит `TerminalClosed` (§7) — и
найденной, и услышанной.

Путь к программе знает агента не всегда. Процесс, чей исполняемый файл уже удалило обновление
Claude Code, `proc_pidpath` не называет (`ENOENT`): так был невидим второй висящий агент, запущенный
14 сентября на 2.1.270. А процесс, чей файл ядро называет по второму имени —
`~/.local/share/claude/ClaudeCode.app/Contents/MacOS/claude`, его заводит фоновая сессия, — правилом
пути (`…/claude/versions/<версия>`) не узнаётся, даже если его запустили обычным образом. Поэтому
агентом процесс признаётся по пути или по собственной записи в реестре Claude Code со сверкой
времени запуска; почему ядро так делает и как устроена сверка — `agent-integration.md` §1б, «Путь к
программе называет файл, а не запуск».
...
(`ClaudeProcessRules.clientKind` о процессе), иначе клик по такой строке искал бы окно там, где
