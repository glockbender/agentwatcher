...
**Кнопка `↗` нажимается всегда, кроме одного случая.** Серый control говорит «до этой сессии не
добраться», что почти никогда не правда: хозяин, как правило, на месте. Насколько близко доводит
одно нажатие — говорит карточка строки: «↗ brings GoLand forward — the mcp-hub window, terminal tab
named above», а когда хозяина действительно нет — «No window to bring forward».

Исключение — **фоновая сессия** (Claude Code запускает такие сам, в своём pty под
`claude bg-pty-host`). У неё окна нет и не появится: замерено, дерево процессов такой сессии
упирается в `launchd`, приложения над ней нет вовсе. Это единственный ответ, который не может
измениться позже, — то самое «решено заранее», которого требовало правило выше, поэтому для неё
кнопка серая. Строка при этом не теряет первую колонку: кнопка остаётся на месте, иначе разъехались
бы колонки всего списка. Почему серая, говорит карточка: «No window to bring forward — a background
session runs in the agent's own pty», а в первой строке карточки стоит `Background` вместо `CLI`.
Отправитель узнаёт такую сессию по команде своего процесса-предка (`AgentProcessLocator`), значок в
строке — перечёркнутое окно.
...
`claude bg-pty-host …`, `claude bg-spare …`. По пути их не отличить, он общий, и каждый из них
