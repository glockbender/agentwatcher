...
Пять групп: лампа (строка на фазу — название, цвет, движение, и подсказка из колонки «Значение»
таблицы §7), фон (палитра из десяти), непрозрачность, размер и сочетание клавиш. Кнопка возвращает
лампе вид приложения.
...
[ADR-0008](adr/0008-the-global-shortcut-goes-through-carbon.md).

Три правила, которые видны человеку:

- **голая клавиша не принимается.** Зарегистрированная на всю систему, одна буква отбиралась бы у
  каждого приложения, включая набор текста. Нужен хотя бы один из `⌃⌥⇧⌘`;
- **пункт меню остаётся и печатает сочетание рядом с собой — но только пока оно работает.** Если
  комбинацию держит другое приложение, меню молчит, а окно настроек говорит, какая именно
  комбинация не взялась и что делать. Меню, обещающее мёртвое `⌥⌘W`, — обещание, которого
  приложение не сдержит;
