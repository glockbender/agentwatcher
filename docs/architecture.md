...
теперь открыты 14 точек её высоты из 22, и знак виден. Про верхнюю капсулу такого числа нет:
с неё спрошено только то, что проверяет тест, — больше половины кнопки остаётся открыто. Вниз капсула опускается ровно настолько, насколько позволяет то, что
