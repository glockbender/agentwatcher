...
состоит. Макет, по которому согласовано: полоса-шаблон, под ней список слотов, над обоими —
образцовая строка на фоне виджета.
...
сессии есть что в них положить. Окно настроек говорит это словами
рядом с каждой частью, а образцовая строка показывает все сразу и подчёркивает пунктиром те,
которых может не оказаться. Без этого часть нельзя поставить на место, не видя её.
...
без) и контекст (процент, токены или оба). Счётчики — один слот, внутри которого галочки по шести
видам; шесть отдельных слотов дали бы шесть почти одинаковых строк списка.

**Что выбирает человек, а что нет.** Сжимаемую часть выбирает он — радиокнопкой среди тех, у кого
есть что сокращать. Что она одна, что разрыв один и что `×` стоит последней — не выбирает.
Отдельной настройкой становится только колонка под `×`: резервировать её во всех строках, чтобы
счётчики не съезжали, когда сессия закончилась, или отдать место содержимому.
...
- [x] Окно настроек проверено так же, как секция лампы (`RowLayoutSectionTests`), без запуска
  приложения.
- [x] Строка отрисована в PNG и просмотрена. Нашлись два дефекта, которых рассуждение не давало:
  колонка «когда часть появляется» была сжата до двух символов, и образец в окне не перерисовывался
  при смене фона. Оба починены и закрыты тестами.
