...
3. **Счётчик скрытых строк не отнимает высоту у списка — сделано.** `▾ N more` стоял в потоке под
   ...
   **Отложено — восстановление прокрутки не работает.** Найдено замером при этой правке, к
   счётчикам отношения не имеет и существует независимо от них. `HUDPanelController` помнит
   `savedScrollOffset` и отдаёт его новому списку, но `restoreScrollOffsetIfNeeded` вызывается из
   `layout()`, а родитель раскладывается раньше детей: в этот момент у документа высота 0.
   Замерено — при запрошенном `y=46` смещение выставляется, а следующий проход возвращает его в 0,
   и одноразовый `hasRestoredScrollOffset` уже израсходован. Значит любая пересборка списка
   (перетаскивание ширины, смена палитры, появление блока usage) роняет прокрученный список
   наверх. Починка не на две строки: у контейнера `layoutSubtreeIfNeeded` этого вида не
   вызывается вовсе, так что нужен другой момент — вероятно однократное наблюдение за кадром
   документа, — и переносить вместе с ним привязку наблюдателя `onScroll` нельзя вслепую. Отдельной
   веткой, со своим замером.
