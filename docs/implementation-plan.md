...
### Настраиваемый формат строки сессии — согласовано 2026-09-15
...
- Строка рисуется по шаблону, и смена шаблона перестраивает весь список, а не только новые строки.
- Инварианты проверены тестами на значении, а не на картинке: один разрыв, одна сжимаемая часть,
  нет дублей, файл из будущей версии читается.
- Перенос `showsSessionTopic` проверен тестом на обоих значениях галочки.
- Окно настроек проверено так же, как секция лампы: выставить значение, послать action, прочитать
  результат — без запуска приложения.
- Строка в нестандартном порядке отрисована в PNG и просмотрена — иначе сжимаемая часть, стоящая
  не на месте имени, проверена только рассуждением.
- `docs/architecture.md` §8 описывает шаблон и формат ключей: ADR-0011 ссылается туда, и ссылка не
  должна вести в раздел, который об этом молчит.
