...
Найдено 2026-09-22 при разборе строки без терминала, закрыто 2026-09-26 — две дыры сканера живых
процессов (§14 архитектуры, «Агент работает, а приложение о нём не слышало»):

- агента, чей исполняемый файл удалило обновление Claude Code, сканер не видит: `proc_pidpath`
  отвечает `ENOENT`. Так остался невидимым агент 2.1.270, висевший с 14 сентября;
...
Обе закрыты одним правилом для сканера и отправителя: агент узнаётся по пути к файлу **или** по
записи `~/.claude/sessions/<pid>.json` со сверкой времени запуска (`ClaudeSessionRegistry`), а обход
предков больше не останавливается на процессе без пути ([интеграция](agent-integration.md) §1б).
Сканер нашёл те же 11 сессий, что и до правки, и агента 2.1.270. Осталось не замеренным, пишут ли
записи помощники агента (`bg-pty-host`, `bg-spare`, `daemon`); отсев помощников действует и на
узнанных по записи.
