...
| 1.5. Session host | **partially complete** | Для CLI известен host-процесс; завершение процесса и фокус приложения работают best-effort. Фоновая сессия Claude открывается по `↗` в новой вкладке Ghostty через `claude attach`. Закрытие сессии определяется тремя способами: `SessionEnd`, завершение процесса или приложения, потеря связи по времени |
...
- кнопка фокуса нажимается всегда, и это решение: она была серой, пока приложение не могло назвать
  ...
  говорит карточка строки словами (§8 архитектуры). Фоновая сессия — та, что живёт в pty агента
  без окна, — по нажатию открывается в новой вкладке Ghostty командой `claude attach <job id>`;
  идентификатор задания читается из `~/.claude/sessions/<pid>.json` в момент нажатия.
...
`com.glockbender.agentwatch.plugin.jetbrains` — сменён 2026-09-12 с `…ide`, пока это ещё ничего не
стоило, потому что после публикации идентификатор не меняется; для IDE плагин с другим
...
Внутренним API (`@ApiStatus.Internal`) плагин не пользуется. Экспериментальным — пользуется, и это
цена работы с переработанным терминалом: `./gradlew verifyPlugin` против GO-261.26222.72 даёт
`Compatible` при 27 использованиях `@ApiStatus.Experimental`. Для Marketplace это предупреждение, а
не отказ. Обещанием стабильности отсутствие ошибок не является: эти интерфейсы вправе поменяться
между релизами, и `verifyPlugin` на новой мажорной версии IDE — способ узнать об этом раньше
пользователя.
