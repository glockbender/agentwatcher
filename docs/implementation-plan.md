...
|---|---|---|
| 1. AppKit MVP | **active dogfooding** | Рабочий HUD получает Claude/Codex события, показывает состояния, активности и свежесть сигналов. У виджета один вход (`WidgetState`) и перерисовка по строкам; приём событий и встраивание вынесены из `AppDelegate`. Значок в строке меню несёт четыре счётчика и отвечает на вопрос «нужен ли я сейчас» при скрытом виджете — [`menu-bar-icon-design.md`](menu-bar-icon-design.md) |
...
- закрытие вкладки терминала в PyCharm и GoLand агента **не** завершает (замерено 2026-09-22 на
  Claude Code 2.1.270 и 2.1.280): он висит в `tcsetattr`, и `SIGTERM` с `SIGKILL` его не берут.
  Такая строка получает фазу `terminal closed` (§7 архитектуры) на ближайшем сканировании или клике,
  а клик по ней завершает агента сбросом непрочитанного вывода терминала
  ([ADR-0013](adr/0013-a-click-ends-an-agent-its-closed-terminal-left-behind.md)). Критерий «закрытие
  … переводит сессию в `session closed` не позднее чем через 10 секунд» для этого случая не
  выполняется: события у закрытия нет, и отметка ждёт ближайшего запуска, пробуждения, открытия меню,
  15-секундного обхода или клика.
- к сессии возвращает клик по её строке — по любому месту, кроме `×`; отдельной кнопки `↗` в строке
  ...
  приложение появлялось. Про обычный клик карточка молчит и говорит только там, где клик сделает
  не то (§8 архитектуры).
...
- сессия, отправленная в фон (`/bg`), не становится второй строкой: Claude Code продолжает её копией
  ...
  он жив, строка кликается как терминальная и карточка называет её `CLI`, а клик поднимает его вкладку, а не открывает
...
разбирает `tool_result`. Обе попадают в файл только когда вызов вернулся — замерено 236 секунд между
меткой времени записи о начале и моментом, когда читатель её увидел, — поэтому на время вызова
молчат оба источника сразу, и порог необъяснённого молчания поднят с двух минут до трёх. Виджет показывает такой вызов как активность `advisor` — значком без числа,
...
- переход к сессии кликом по её строке, best-effort;
...
Отдельно от фаз: **работа, оставшаяся после хода** — фоновая команда, сабагент, монитор. Приходит
списком в `Stop` (`agent-integration.md` §3), показывается счётчиком у строки `готово` и не меняет
фазу ([ADR-0008](adr/0008-background-work-does-not-claim-the-turn.md)). До этого команда, которую
Claude Code сам перевёл в фон по таймауту, не была видна нигде.
...
- точный переход используется только тогда, когда окно и вкладку можно назвать достоверно;
...
- вызов `Agent` тоже возвращается почти сразу (медиана 2.1 с на 152 вызовах), поэтому сабагента
  ...
  родительской сессии. Это же поле отвечает на вопрос, чей диалог висит: ожидание принадлежит
  одному агенту, и события остальных его не гасят (`agent-integration.md` §1в);
...
- **правило «закрытую строку не трогать» было написано шесть раз**; теперь один `liveRow(_:)`, и
  седьмой метод получает правило даром. Рядом живёт другое правило — закрытую строку заново
  открывает только старт, — оно написано трижды и осталось: предикат у него другой
  ([ADR-0002](adr/0002-a-closed-session-is-terminal.md));
...
### Продолжение ревью — 2026-09-15

- **`AgentWatchSender` разделён по вопросу, на который отвечает.** `AgentWatchLookup` спрашивает
  машину (`AgentProcessLocator`, `AgentProcessScanner`, `SessionDescriptionResolver`,
  `SessionProcessExitWatcher`), `AgentWatchSender` собирает запрос и отдаёт его сокету. Резать было
  нечего: между половинами не было ни одной связи. У каждой теперь свой тестовый таргет — пять
  тестовых файлов перестали жить в `AgentWatchIngressTests`, который сохранил зависимость от
  отправителя намеренно: `UnixSocketIngressTests` проверяет сокет сквозным путём, настоящим
  `HookEventSender.send`, а не сообщением, которое написал сам. Разрез вскрыл мёртвый импорт в
  `ToolingCoordinator` и комментарий `SessionDescriptionResolver`, утверждавший, что тот работает
  только в hook-процессе, — приложение вызывает его тоже;
- **`AgentWatchIngress` — ровно сервер сокета.** `SessionProcessExitWatcher` наблюдал за процессом
  через `DispatchSource` и не знал ни про Core, ни про сокет; он уехал к остальному спрашиванию, и
  `SessionHostRegistry` больше не импортирует `AgentWatchIngress`;
- **`docs/adr/` — семь решений**, отобранных по двум признакам: на решение ссылается код, либо ревью
  его отклонило и следующее ревью предложит снова. Код ссылается `ADR-0001` вместо
  `docs/architecture.md` §15 — номер раздела съезжает при вставке выше, номер ADR нет. Проза
  осталась на месте, разделы получили указатель в обратную сторону. Каждый ADR цитируется из кода,
  которым он управляет, — иначе он украшение;
- **`docs/measurements.md` — индекс замеров по версиям.** Замеры остаются там, где объясняют
  решение; таблица отвечает на другой вопрос — что перепроверить, когда обновится агент. Где в
  исходном тексте версии нет, так и написано: такой замер к обновлению не привязан.
...
2. **Описание сессии догоняется из транскрипта — сделано.** При запуске восстановленная строка
   ...
   - **восстановление фазы `waitingForUser` — сделано.** Память говорит, какая фаза была,
     транскрипт — держится ли она ещё. Проверок в итоге две, а не одна: у каждого запомненного
     диалога появился результат его вызова — ожидание кончилось; в хвосте есть любой факт
     новее момента ожидания — сессия жила дальше, и результат мог не попасть в окно чтения.
     Диалогов запоминается столько, сколько было: ответ на один из двух ожидание не снимает.
...
3. **Счётчик скрытых строк не отнимает высоту у списка — сделано.** `▾ N more` стоял в потоке под
   ...
   список занимает всю высоту виджета всегда, а счётчик рисуется поверх него капсулой в правом
   нижнем углу: овал с минимальным полем вокруг надписи, шрифт меньше всего остального в виджете,
   ...
   Две поправки по жалобе: считаются только строки *ниже* видимой области — раньше в счёт шли и
   ушедшие вверх, и внизу списка капсула объявляла уже прочитанные сессии под стрелкой вниз; и сама
   капсула ушла в рамку виджета и стала чуть меньше, потому что закрывала кнопку удаления сессии
   целиком, а теперь открыта большая часть её высоты.

   Ещё поправка: счётчиков стало два, по одному на сторону, и надпись сократилась до `+N`. Тот
   счёт, что остался «только вниз», молчал про строки, ушедшие вверх, — а они так же скрыты, и
   внизу списка виджет не говорил о них ничего. Складывать обе стороны в одно число нельзя, это и
   была исходная жалоба, поэтому счёт разделён: `hiddenRows` возвращает `above` и `below`, каждая
   капсула показывает свою и прячется отдельно. Стрелка из надписи убрана — угол говорит
   направление сам. Верхняя капсула стоит на одном месте всегда: над списком не бывает ничего,
   кроме края виджета. Пишется только та надпись, чей счёт изменился: прокрутка на строку обычно
   двигает одну сторону, а запись в метку метит её к перерисовке и заново размывает панель.

   И порог: скрытой строка считается, когда за край ушло больше 30 % её высоты, а не с первой
   обрезанной точки. Жалоба была ровно на это — виджет на три точки короче, последняя строка
   читается целиком, а капсула над ней обещает продолжение, которого нет. Доля, а не точки:
   подстраивается под высоту строки и заодно покрывает полуточечный промах кадра, ради которого
   раньше стоял отдельный допуск. Считается по каждому краю отдельно. Порог проверен с обеих
   сторон на точных числах (30 % от 20 точек — ровно 6: шесть ещё видно, шесть с половиной уже
   нет) и на настоящей раскладке: виджет на три точки короче молчит, на восемь — считает.

   Проверено вместе с масштабом виджета, после слияния с ним. Порог — доля от настоящего кадра
   строки, поэтому за размером он следует сам: 3,6 точки на половинном, 11,4 на двойном, и
   граница везде падает в одно и то же место относительно строки. Капсулы друг на друга не
   налезают ни на одном размере. Единственное, что размер меняет всерьёз, — сколько знака `×`
   остаётся видно под верхней капсулой; числа и решение оставить как есть — в
   `docs/architecture.md` §8.

   Контекстные курсоры из этого пункта убраны: кромка уже отвечает подсветкой, а остального
   достаточно как есть.

   **Восстановление прокрутки — сделано.** Найдено замером при правке счётчиков, к ним
   отношения не имеет и существовало с первого коммита: `HUDPanelController` помнит
   `savedScrollOffset` и отдаёт его новому списку, но `restoreScrollOffsetIfNeeded` вызывался
   из `layout()`, а родитель раскладывается раньше детей — в этот момент у документа высота 0.
   Замерено: при запрошенном `y=46` смещение выставлялось, следующий проход возвращал его в 0,
   и одноразовый `hasRestoredScrollOffset` был уже израсходован. Любая пересборка списка
   (перетаскивание ширины, ползунок размера, смена палитры, появление блока usage) роняла
   прокрученный список наверх.

   Правильный момент найден замером, а не рассуждением: инструментированы `layout`,
   `layoutSubtreeIfNeeded`, `updateConstraints`, `viewWillDraw`, `setFrameSize`,
   `viewDidMoveToWindow` и кадр самого документа — на настоящей связке со `HUDContentContainer`
   в показанном окне. Первым, где у документа есть высота, приходит его собственный
   `frameDidChange`, и приходит он внутри того же прохода, поэтому наверху ничего не успевает
   отрисоваться. Вариант «звать из `layoutSubtreeIfNeeded`» отпал там же: у списка этот метод
   не вызывается вовсе, раскладкой правит контейнер.

   Заодно вскрылось второе: `NSClipView.scroll(to:)` смещение **не** зажимает — запрос на 4000
   точек в списке высотой 65 уводил на 4000 и показывал пустоту. Теперь зажимается через
   `constrainBoundsRect`, то есть список, ставший короче с прошлого раза, встаёт настолько
   вниз, насколько теперь может, а не отказывается вовсе. Порядок привязки наблюдателя
   сохранён: он подписывается только после восстановления, иначе собственное движение
   восстановления пришло бы обратно как прокрутка человека и затёрло бы то самое смещение.

   Прокрутка внутри чужого прохода раскладки — та самая вещь, что однажды подвесила виджет,
   поэтому проходы считаются тестом, а не предполагаются.
4. **Фокус на конкретной сессии — ступень «назвать словами» сделана.** Разбор вариантов — в
   ...
   Сделано: `SessionReach` считается в момент наведения и говорит, есть ли кого поднимать. Окно и
   вкладку внутри IDE выбирает плагин по номеру процесса агента; для Ghostty вкладка ищется по
   имени, которое агент сам пишет в заголовок терминала. Ни одного разрешения macOS. Приложение
   какое-то время читало ещё и `recentProjects.xml` самой IDE, чтобы карточка могла назвать окно
   проекта, — эта строка карточки убрана, и разбор вместе с ней. Переход доступен всегда: прошлое
   ...
   Из этого перехода приходил и отчёт владельца: фокус попадал на верную вкладку, но вперёд
   выходили все окна Ghostty разом — подъём хоста шёл с `activateAllWindows`, а точный фокус
   ложился поверх той кучи. То же было верно для окон IDE, где это видно только при нескольких
   открытых проектах. Правило переписано: маршрут до вкладки вычисляется до подъёма, широкая сеть
   остаётся только там, где вкладку не адресовать ничем. Правило под тестом
   (`SessionFocusWindowsTests`), и владелец проверил его на живых окнах: вперёд выходит только окно
   нужной вкладки. Разбор — `docs/architecture.md` §8.
...
5. **Масштаб виджета — сделано.** Ползунок `Size` в окне настроек: от 50 до 200 процентов шагом в
   пять процентов, с засечками и только на них. Шаг был в четверть, пока владелец не попросил
   мельче; к засечке округляет и само хранилище, чтобы отредактированный рукой файл настроек не
   задал размер, которого в списке нет. Ползунок растянут во всю ширину вкладки и растёт вместе с
   окном — 31 засечка на прежних 260 точках сливалась в частокол. Растут все шрифты, все значки, лампа, высота строки и
   отступы: масштаб несёт одно значение `WidgetStyle`, которое виджету передают так же, как фон и
   схему ламп. Смена масштаба перестраивает всё, что на экране, — иначе человек тянул бы ручку и
   ничего не происходило, — и двигает нижнюю границу размера окна. Двух вещей масштаб не касается:
   радиусов скруглений и порога, после которого нажатие становится перетаскиванием. Проверено
   отрисовкой офскрин на каждом шаге — там и нашлись все три места, где AppKit рисует своим
   размером, а не выданным: рамка кнопки `×` вверх и вниз, пол у маленьких символов и запас ярлыка
   вокруг текста. Устройство — `docs/architecture.md` §8.
6. **Сочетание клавиш для показа и скрытия виджета — сделано.** `⌥⌘W` из любого приложения,
   свежая установка получает его сразу, в окне настроек записывается другое. Голая клавиша
   отвергается: зарегистрированная на всю систему, она отбиралась бы у набора текста. Пункт меню
   остался и печатает сочетание рядом с собой, но только пока регистрация держится, — иначе меню
   обещало бы то, чего нет, а объясняет причину окно настроек. Почему Carbon, а не современные
   API — [ADR-0009](adr/0009-the-global-shortcut-goes-through-carbon.md).

   Проверено не тестом, а на запущенной копии: окна процесса в списке экрана, нажатие настоящим
   событием `CGEventPost`, виджет уходит и возвращается. Там же опровергнуто ожидание, что хоткей
   и `keyEquivalent` того же пункта сработают дважды при открытом меню, — срабатывает один, и
   глушилка, которую я собирался поставить, не написана. Запись другого сочетания проверена
   руками владельца на той же копии: набранное в окне `⌥⌘E` легло на диск и стало работать, а
   перерегистрация поверх прежнего `⌥⌘W` прошла без отказа «занято».

   Там же опровергнуто второе ожидание — про конфликт с чужим приложением. Его не существует:
   два процесса регистрируют одно сочетание, обоим `noErr`, нажатие получают оба. Значит
   `eventHotKeyExistsErr` может означать только собственную неснятую регистрацию, и случай
   переименован из `taken` в `alreadyOurs`, а надпись в окне больше не говорит про чужое
   приложение. Тогда же тест на высоту строки состояния нашёл дефект, который был там с самого
   начала: строке выделялась одна строка текста, а четыре из пяти сообщений в неё не влезали.
   Голая клавиша отвергается везде, кроме `F1`–`F19`: замер показал, что система против неё
   ничего не имеет, а в текст F-клавиши не набирают. Стрелки под исключение не попали — приходят
   они так же, а нажимают их постоянно. Устройство — `docs/architecture.md` §8, замеры — в
   `docs/measurements.md`.
7. **Свой цвет фона — сделано.** Под палитрой из десяти в окне настроек — строка `Custom`:
   колодец цвета, который открывает системную панель сразу на цветовом круге и одним нажатием
   делает свой цвет фоном. Цвет хранится отдельным ключом и переживает переход на пресет. Тёмный
   или белый текст на нём выбирается по контрасту WCAG, и тем же правилом теперь считаются пресеты
   — тест держит его согласным с рядами палитры. По дороге найдена ловушка AppKit: колодец,
   созданный через `init(style:)`, оказывается обычным `NSColorWell`, и переопределение молча не
   работает; тест поэтому кликает настоящим событием мыши. Устройство — `docs/architecture.md` §8.

### Настраиваемый формат строки сессии — сделано 2026-09-15

Порядок частей строки перестаёт быть списком в коде и становится настройкой. Решение и его
границы — [ADR-0011](adr/0011-the-row-is-a-template-with-one-gap.md); здесь то, из чего оно
состоит. Согласовано по макету, где над списком частей стояла образцовая строка на фоне
виджета; полосу-шаблон из макета заменил сам список — все контролы стоят в строке своей части.

**Части.** Тринадцать: `timer`, `lamp`, `agent`, `fault`, `name`, `project`, `branch`, `model`,
`host`, `thread`, `counters`, `context` и `gap`. Таймер, лампа, иконка агента и разрыв есть у
каждой строки; остальные — включая маркер проблемы, который как раз редок, — появляются, когда
сессии есть что в них положить. Про каждую часть это сказано словами в колонке `appears`,
целиком — в подсказке при наведении: без этого часть нельзя поставить на место, не зная, увидишь
ли ты её вообще.

**Что настраивается у части.** Показывать или нет; место в порядке; у трёх — вид: имя
(`Name, else project` — нынешнее поведение, или `Name only`), модель (с усилием рассуждения или
без) и контекст (процент, токены или оба). Счётчики — один слот, внутри которого выпадающее меню
с галочками по шести видам работы; шесть отдельных слотов дали бы шесть почти одинаковых строк
списка и похоронили бы в них остальные двенадцать частей.

**Что выбирает человек, а что нет.** Сжимаемую часть выбирает он — радиокнопкой в колонке
`gives way`, у каждой части, которой есть что сокращать. Что она одна, что разрыв один и что `×`
стоит последней — не выбирает. Отдельной настройкой становится только колонка под `×`:
резервировать её во всех строках, чтобы счётчики не съезжали, когда сессия закончилась, или отдать
место содержимому. Образцовых строк поэтому две — работающая и завершённая: у работающей кнопки
нет, и разницу видно только их сравнением.

**Где живёт значение.** `RowLayout` — в `AgentWatchApp`, рядом с `LampScheme`: это конфигурация
отрисовки, а не состояние сессии, поэтому правило AGENTS.md про `AgentWatchCore` его не касается.
Тесты — в `AgentWatchAppTests`, как у схемы лампы.

**Хранение.** Плоскими ключами в `settings.json`, рядом со схемой лампы и в том же стиле:
`rowLayout.parts` — порядок через запятую, `rowLayout.flexible`, `rowLayout.nameStyle`,
`rowLayout.modelStyle`, `rowLayout.contextStyle`, `rowLayout.counterKinds` — тоже через запятую, и
`rowLayout.reservesDismissColumn`. Причина не в стиле: `PreferenceFile` умеет строку, число и флаг
и не умеет массив, а расширять общий файл настроек ради одной настройки дороже, чем записать
порядок строкой, которую вдобавок видно глазами. По тому же правилу, что и лампа, файл держит
конфигурацию целиком — все ключи пишутся всегда.

Инварианты чинятся при чтении, а не проверяются при рисовании: нет разрыва — добавляется в конец,
их несколько — остаётся первый, сжимаемая часть невалидна — берётся первая подходящая, неизвестная
часть пропускается молча, дубли схлопываются до первого вхождения.

**`showsSessionTopic` удалён.** Он становится частным случаем шаблона, и строка из меню уходит,
чтобы две настройки не спорили об одном. Без переноса значения: приложение не выпущено, переносить
не с чего, а код переноса — это вечная ветка ради состояния, которого ни у кого нет.

Флаг читает не только строка: `hoverCardText` берёт его, чтобы не показать в карточке имя, которое
строки только что перестали показывать. Он переходит на тот же источник — «есть ли `name` в
шаблоне», — и обещание остаётся одно, а не два.

**Пустая строка при `Name only`.** У сессии без имени эта часть не нарисует ничего, и строка
останется лампой и часами — ровно то, что нынешняя подстановка `[still no name] in <проект>`
предотвращает. Принимается как выбор человека: вариант называется `Name only` и делает то, что
написано; второй подстановки под ним нет. Кому нужны оба — включает отдельную часть `project`.

**Границы.** Не входит: перетаскивание частей мышью, готовые пресеты, разные шаблоны для Claude и
Codex, произвольный текст между частями. Причины — в ADR-0011.

**Критерии готовности — все выполнены.**

- [x] Строка рисуется по шаблону, и смена шаблона перестраивает весь список: шаблон лежит на
  `HUDRowModel`, иначе два шаблона с одинаковым именем и одинаковой кнопкой сравнялись бы равными.
- [x] Инварианты проверены тестами на значении, а не на картинке (`RowLayoutTests`).
- [x] Засев пишет шаблон целиком, а уже записанный шаблон переживает следующий запуск
  (`RowLayoutStoreTests`, `PreferenceSeedingTests`).
- [x] Окно настроек проверено так же, как секция лампы (`RowLayoutSectionTests`), без запуска
  приложения. Каждая настройка имеет контрол: виды счётчиков — выпадающее меню с галочками,
  сжимаемая часть — радиокнопка в своей колонке у каждой части, которой есть что сокращать.
- [x] Выбранный шаблон доходит до виджета и перестраивает строки сразу
  (`RowLayoutTakesEffectTests`) — шов между окном и виджетом, который обе стороны пропускали.
- [x] Строка и окно отрисованы в PNG и просмотрены. Нашлись дефекты, которых рассуждение не
  давало: колонка «когда часть появляется» была сжата до двух символов; образец не перерисовывался
  при смене фона; окно было выше экрана ноутбука и на 20 точек уже собственных контролов. Все
  починены и закрыты тестами.
- [x] `docs/architecture.md` §8 описывает шаблон и формат ключей — раздел «Из чего состоит строка,
  и кто это решает».

**Что нашёл первый запуск — 2026-09-16.** Два дефекта, которых не увидели ни тесты, ни отрисовка.

Первый: контролы копились стопкой. `NSGridView.removeRow(at:)` убирает строку из сетки, но оставляет
её контролы в окне, и они продолжают рисоваться там, где стояли. Каждая перестройка списка частей
добавляла копию — одну при создании окна и по одной на каждый визит и каждый клик. Совпадающие копии
читаются просто как жирный текст; стоило переставить часть — старый порядок и новый рисовались друг
поверх друга. Тесты этого не видели, потому что достают контролы через словари контроллера, а там
лежит только последняя копия; отрисовка — потому что рисовала окно сразу после создания, где копии
ещё одинаковые. Теперь тест считает то, что есть в окне.

Второй: окно было слишком большим. Одна колонка из шести групп просила высоту всех групп сразу, и
окно открывалось во весь экран. Группы разложены по трём вкладкам — `Row` (формат строки и размер),
`Lamp`, `Other` (фон, непрозрачность, сочетание клавиш), — и окну хватает самой высокой из них: 603
точки вместо 1290. Устройство и следствия (контрол невидимой вкладки не в окне, поэтому запись
сочетания клавиш живёт на своей вкладке и отменяется уходом с неё) — `docs/architecture.md` §8.

И одна просьба вдогонку: смена размера зажигает контур виджета — тот же, что у строки меню
`Highlight Widget`. Ползунок теперь стоит рядом с образцом строки, и человек смотрит на него, а
меняется окно где-то ещё. Следом за ним — `Reset Widget Size` и `Reset Widget Position`: те же
условия, только человек смотрит в меню. Масштабировать образец строки вслед за ползунком владелец
отказался: масштабируется сам виджет, этого достаточно, а образец тянул бы за собой высоту вкладки.
`docs/architecture.md` §8.

Третий — в консоли Xcode, а не на экране: `Conflicting constraints detected` на заданные ширины
колонок сетки частей. Вкладки попадали в окно до того, как окно измеряли, и раскладывались один раз
при ширине рождения окна (400 точек минус рамка вкладок), где колонки не помещаются. Теперь окно
получает размер до того, как вкладки в него попадают, а требование «содержимое шириной во всё окно»
стало пожеланием: вкладка держит нужную ей ширину, остальное добирается прокруткой вбок. Сам текст
ошибки в тестовом процессе не печатается — тест держит вкладку узкой через ограничение и проверяет
геометрию.

И второй вопрос от первого запуска: зачем в настройке строки две почти одинаковые строки. Пара
нужна — без неё `Keep the × column` не показывает ничего, — поэтому под ней теперь стоит строчка,
которая говорит, что это одна и та же сессия в работе и после конца. `docs/architecture.md` §8.

**Что нашло ревью — 2026-09-16.** Просмотр всей фичи целиком: код, тесты, доки. Пять правок, две
из них — контролы, которые приложение предлагало и не выполняло.

- **Стрелки у частей вне строки** были живыми и не двигали ничего: восемь из десяти на пяти
  частях, проверено нажатием. Порядок хранится только для частей строки; остальные перечислены в
  том порядке, в каком их называет приложение, и он нигде не лежит. Теперь серые, а подсказка
  отправляет к галочке — единственному контролу, который там что-то меняет.
- **Последнюю часть можно было выключить.** Шаблон из одного разрыва рисует в каждой строке полоску
  в 33 точки (замерено на образце), а вернуться можно было только кнопкой, сбрасывающей всё
  остальное. Галочка последней оставшейся части выключена и говорит почему.
- **Арифметика ширины при сжимаемой части после разрыва** была нарисована и просмотрена глазами
  (`branch-gives-way` в пробнике отрисовки), но не закреплена утверждением — а пробник сам про это
  и писал. Тест есть: строка помещается в ширину, на которую её мерили, и та же строка с полной
  веткой — нет.
- **`setLayout` переписывал `settings.json` семь раз на клик**, по разу на ключ, потому что
  `PreferenceFile` сбрасывает документ на каждый `set`. Один `replace`, как у лампы при сбросе.
- **Правило сказано по одному разу.** «Имя своим цветом, остальное приглушённо» стояло в двух
  функциях; значения по умолчанию у `layout` и `style` повторяли ровно ту ловушку, которую автор
  осознанно убрал у `hoverCardText`; `drawnOn` был обёрткой над полем ради одного теста.

**Что осталось за рамками и почему.** Перетаскивание частей мышью: порядок меняется стрелками,
потому что это тот же приём, что уже стоит в окне, и его есть чем проверить в тесте. Пресетов нет —
владелец отклонил их как лишние при полной свободе выбора частей; вместо них одна кнопка
`Use the app's own row`. Отдельных шаблонов для Claude и Codex нет: ADR-0011.
...
Найдено 2026-09-22 при разборе строки без терминала, в очередь не поставлено — две дыры сканера
живых процессов (§14 архитектуры, «Агент работает, а приложение о нём не слышало»):

- агента, чей исполняемый файл удалило обновление Claude Code, сканер не видит: `proc_pidpath`
  отвечает `ENOENT`. Так остался невидимым агент 2.1.270, висевший с 14 сентября. Направление —
  путь исполняемого файла в начале `KERN_PROCARGS2`, он переживает удаление файла; не проверено;
- агента, чей файл ядро называет по второму имени, правило пути (`…/claude/versions/<версия>`) не
  узнаёт. Второе имя, `~/.local/share/claude/ClaudeCode.app/Contents/MacOS/claude`, заводит хост
  фоновой сессии, и после его старта так выглядят все сессии той же версии — до следующего запуска
  по старому имени, а когда обновление удалит версию, навсегда
  ([интеграция](agent-integration.md) §1б, «Путь к программе называет файл, а не запуск»). Так
  выглядела обычная сессия 2.1.272 в `mcp-hub`, и её строка держалась только на хуках. Отправитель
  узнаёт агента по тому же правилу (`ClaudeProcessRules.agentProcessID`), поэтому хуки
  таких сессий приходят без номера процесса: ни наблюдения за выходом, ни проверки терминала. У
  этой сессии номер, видимо, остался от хуков до 22:24 — не проверено.

Направление, которое закрывает обе, — узнавать агента по пути к файлу **или** по записи
`~/.claude/sessions/<pid>.json` со сверкой времени запуска, одним правилом для сканера и
отправителя. Запись дополняет путь, а не заменяет: сейчас она есть у всех 11 живых сессий на этой
машине, включая 2.1.270, но бывает не у каждой ([интеграция](agent-integration.md) §1б). Пишут
ли записи помощники агента (`bg-pty-host`, `bg-spare`, `daemon`), не замерено, поэтому отсев
помощников остаётся и для узнанных по записи. Не проверено.

Правила агента по слоям ([интеграция](agent-integration.md) §1а): слой `AgentWatchLookup`
переведён 2026-09-26 — `AgentProcessRules`, `ClaudeProcessRules`, `CodexProcessRules`, без
изменения поведения. Остальные переводятся, когда их трогают:

- чтение транскрипта и имени сессии — `TranscriptLocator`, `TranscriptReader`,
  `SessionDescriptionResolver` и его ветка Codex в отправителе;
- запись реестра Claude: ядро состояний принимает `ClaudeSessionStatus` и проверяет
  `source == .claude` (`SessionStateEngine.apply`), а должно принимать общий факт «на диалог
  ответили»;
- установка хуков и значок.
