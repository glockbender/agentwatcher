...
|---|---|---|
| 1. AppKit MVP | **active dogfooding** | Рабочий HUD получает Claude/Codex события, показывает состояния, активности и свежесть сигналов |
| 1.5. Session host | **partially complete** | Для CLI известен host-процесс; завершение процесса и фокус приложения работают best-effort. Закрытие сессии определяется тремя способами: `SessionEnd`, завершение процесса или приложения, потеря связи по времени |
...
| 3. Установка и обновление | **partially complete** | Установка интеграций из приложения сделана (окно `Tooling…`, состояние «не установлено / установлено / установлено, но ничего не приходило / не хватает хуков / отправитель исчез / файл не читается», полное снятие). Осталось: готовый пакет и автопроверка версии |
...
поднять окно host-приложения по кнопке `↗`.
...
- `↗` активирует host application (GoLand, Ghostty и т. п.) best-effort;
...
- Codex Desktop получает кнопку фокуса на приложение и не получает per-session process watcher: его
...
- кнопка фокуса нажимается всегда, и это решение: она была серой, пока приложение не могло назвать
  окно, а ответ на этот вопрос вычислялся один раз при первом хуке и больше не пересматривался —
  строка оставалась мёртвой и после того, как её приложение появлялось. Что даст одно нажатие,
  говорит карточка строки словами (§8 архитектуры).

### Критерии готовности

- закрытие обычного Claude в GoLand Terminal переводит связанную сессию в `session closed` не
  позднее чем через 10 секунд;
- кнопка `↗` активирует корректное host application или честно недоступна;
...
- кнопка перехода на каждой сессии и best-effort `SessionLocator`;
...
- кнопка перехода как минимум активирует правильное host application;
...
- подготовить подписываемый release artifact приложения;
- определить канал публикации и manifest версий;
- добавить проверку checksum/signature;
- проверять наличие новой версии при каждом запуске и предлагать обновление с возможностью
  отказаться;
- вложить в приложение plugin с актуальными hook-скриптами;
- определять, какие агентские инструменты установлены, и предлагать установку обвязки отдельно для
  каждого;
- повторять проверку при каждом запуске и показывать список установленных и неустановленных
  инструментов;
- запоминать выбор пользователя и возвращаться к вопросу только при появлении нового инструмента;
- вынести ту же настройку в меню, чтобы выбор можно было изменить;
- проводить пользователя через trust-step Codex hooks;
- открывать окно по центру основного экрана при первом запуске;
- безопасно координировать обновление и перезапуск работающего приложения.
...
4. **Фокус на конкретной сессии — ступень «назвать словами» сделана.** Разбор вариантов — в
   ...
   Сделано: `SessionLocator` считается в момент нажатия и называет приложение, проект и вкладку.
   Проект берётся из `recentProjects.xml` самой IDE, вкладка — из имени сессии, которое агент сам
   пишет в заголовок терминала. Ни одного разрешения macOS. Кнопка `↗` теперь нажимается всегда:
   прошлое устройство гасило её по ответу, разрешённому однажды и больше никогда не пересчитанному.
   Устройство — `docs/architecture.md` §8.

   Прыжок до вкладки в JetBrains — тоже сделано, и проверено нажатием `↗` в собранном бандле:
   ...
   нужная вкладка поднимаются одним нажатием. Той же проверкой закрыт и вопрос подписи — после
...
1. Пакет для раздачи — **сделан наполовину, и вторая половина стоит на решении, которое не наше.**
   Сделано: `task release` собирает `AgentWatch-<версия>.zip` и контрольную сумму, тег `vX.Y.Z`
   поднимает тот же сбор в CI и оставляет черновик релиза, версия живёт в одном месте
   (`CFBundleShortVersionString`), и тег с ней сверяется. Не сделано: нотаризация — для неё нужен
   сертификат Developer ID, то есть платная Apple Developer Program, и без него писать этот код
   нечем проверить. Пока релиз подписан ad-hoc, и скачавшему нужна одна команда (`xattr -dr`),
   сказанная и в README, и в примечаниях к релизу. Проверка новой версии при запуске — **сделана**,
   вместе с установкой обновления самим приложением: скачивание, сверка контрольной суммы,
   распаковка, атомарная замена бандла и перезапуск; проверено на настоящем релизе. Разбор — в
   [дистрибуции](distribution.md) и [архитектуре](architecture.md) §11.
2. Проверить чистую установку, обновление, отключение и удаление на обеих интеграциях.
3. Автоматизировать то, что можно, в Codex trust-step, и сказать в интерфейсе про остальное.
4. Добавить smoke-тест: приложение запущено / не запущено.
5. Подтвердить на dogfooding matrix для plan mode, approval, choice, subagent и background activity;
   неподтверждённое не отображать как факт.

### Публикация плагина IDE в Marketplace — основной путь, но не сейчас
...
Не сейчас: пока плагин полируется локально, где круг «собрал — поставил — проверил» короче любого
ревью. Первую версию люди из JetBrains смотрят руками, и тратить это на сборку, которая меняется
каждый день, незачем. Установка из файла при этом сделана и остаётся — она нужна и после публикации:
человеку, который ставит сборку до релиза, и тому, у кого Marketplace закрыт политикой компании.
...
Что уже готово, проверено по собранному артефакту: уникальный идентификатор, `since-build` без
верхней границы, зависимость только от платформы и встроенного терминала, 38 КБ при потолке в
400 МБ. Внутренним API (`@ApiStatus.Internal`) плагин не пользуется. Экспериментальным —
пользуется, и это цена работы с переработанным терминалом: `./gradlew verifyPlugin` против
GO-261.26222.72 даёт `Compatible` при 27 использованиях `@ApiStatus.Experimental`. Для Marketplace
это предупреждение, а не отказ. Обещанием стабильности отсутствие ошибок не является: эти
интерфейсы вправе поменяться между релизами, и `verifyPlugin` на новой мажорной версии IDE — способ
узнать об этом раньше пользователя.

Что нужно сделать, по возрастанию цены:

1. **Файл `LICENSE` в репозитории.** Нужен и без Marketplace, а там лицензия обязательна: открытая
   снимает возню с собственным пользовательским соглашением, достаточно ссылки на исходники.
2. **`<vendor>` с почтой и сайтом.** Одна строка в `plugin.xml`. Оба поля обязательны и должны
   работать — битые ссылки названы причиной отказа прямо.
3. **Описание для витрины.** На английском, грамотное. Первой строкой — что плагин ничего не делает
   без Agent Watch, потому что ревью смотрит, не вводит ли описание в заблуждение. Там же сказать
   про файл ответа, который плагин пишет в каталог Agent Watch: персональных данных в нём нет, но
   нынешнее «stores nothing» уже неправда.
4. **Логотип** SVG 40×40, не шаблонный и не похожий на логотипы JetBrains.
5. **Подпись плагина.** Формально необязательна, но без неё IDE при установке показывает
   предупреждение. Ключ RSA со своей цепочкой сертификатов и задача `signPlugin`, которая
   срабатывает перед публикацией сама. С подписью macOS-бандла никак не связана.
6. **Профиль вендора и загрузка**, дальше ручное ревью. Сроков JetBrains не публикует — в
   документации сказано только, что первая версия проверяется людьми.

Отдельно уточнить на месте: попадаем ли мы под trader verification. Критерий описан общо, для
бесплатного плагина от частного лица он, скорее всего, не нужен, но это догадка, а не факт.
...
- канал доставки обновлений. Сам формат публикации решён: подписанный и нотаризованный bundle, а
  не Mac App Store — правило 2.4.5 запрещает приложению из Store ставить код в чужие места, то есть
  ровно то, ради чего существует окно тулинга. Разбор — в [архитектуре](architecture.md);
...
- переход к вкладке в IDE закрыт целиком: плагин (`ide-plugin/`, цель `task plugin`) берёт вкладки
