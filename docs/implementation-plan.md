...
**`showsSessionTopic` уходит.** Он становится частным случаем шаблона. Значение переносится один
раз при первом чтении: если `rowLayout` в файле нет, а галочка была снята — шаблон получается
без части `name`. Строка из меню убирается, чтобы две настройки не спорили об одном.
...
- [x] Перенос `showsSessionTopic` проверен на обоих значениях галочки и на файле, где шаблон уже
  записан (`RowLayoutStoreTests`).
